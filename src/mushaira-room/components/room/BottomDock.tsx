  ...
  return (
    <div className="mr-fixed mr-bottom-0 mr-left-0 mr-right-0 mr-z-40 mr-border-t mr-border-mr-gold/20 mr-bg-mr-bg/90 mr-px-3 mr-py-3 mr-backdrop-blur-xl">
      <div className="mr-relative mr-mx-auto mr-flex mr-max-w-2xl mr-items-center mr-justify-between mr-gap-2">
        ...
        <button
          onClick={hasSeat ? onToggleMic : onCheckIn}
          disabled={hasSeat && blocked}
        ...
        >
          {hasSeat ? (micOn && !blocked ? '🎙️' : '🎤') : '🪑'}
          <span className="mr-text-[9px] mr-font-medium">{hasSeat ? (micOn && !blocked ? 'On' : 'Off') : 'Seat'}</span>
        </button>
        ...
        >
          ✋
          <span className="mr-text-[9px] mr-font-medium">Raise</span>
        </button>

        <button
          onClick={hasSeat ? onToggleMic : onCheckIn}
          className="mr-flex mr-h-12 mr-flex-1 mr-items-center mr-justify-center mr-rounded-2xl mr-bg-mr-purple-gradient mr-px-3 mr-text-sm mr-font-bold mr-text-white mr-shadow-mr-purple-glow"
        >
          {hasSeat ? 'On Stage' : 'Request to Speak'}
        </button>
